...
"""
Launch file for Agilex Piper arm with joint position control.
...
- joint_position_controller: Provides direct position command control
...
Usage:
  ...
  ros2 launch agilex_piper_arm_bringup agilex_piper_joint_position_control.launch.py can_interface:=can1

  # For arm-only configuration (without gripper):
  ros2 launch agilex_piper_arm_bringup agilex_piper_joint_position_control.launch.py include_gripper:=false

  # For SIMULATION/TESTING without physical robot (RECOMMENDED for testing):
  ros2 launch agilex_piper_arm_bringup agilex_piper_joint_position_control.launch.py use_mock_hardware:=true
...
Test extended arm features with GPIO controller:
  ...
  # Set built-in Cartesian motion mode
  ros2 topic pub --once /agilex_piper_gpio_controller/commands
    control_msgs/msg/DynamicInterfaceGroupValues
...
def launch_setup(context, *args, **kwargs) -> List[Node]:
    ...
    robot_description_raw = xacro.process_file(
        ...
        mappings={
            ...
            'motion_mode': '1',  # Joint mode for joint position control
            'speed': speed_value
        }
    ).toxml()
...
def generate_launch_description() -> LaunchDescription:
    """Generate launch description for Agilex Piper arm with joint position control."""
    # Declare arguments
